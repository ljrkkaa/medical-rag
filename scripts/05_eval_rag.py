...
def main():
    ...
    eval_data = load_dataset(
        "json", data_files="data/eval/new_qa_200.jsonl", split="train"
    )
    qwen_llm = ChatOpenAI(
    ...
    qwen_embedding = DashScopeEmbeddings(
        model="text-embedding-3-small", dashscope_api_key=os.getenv("DASHSCOPE_API_KEY")
    )
    ...
    print(
        eval.do_evaluate(
            datasets_query_field_name="new_question",
            datasets_reference_field_name="answer",
        )
    )
